...
class Crop_Info (Base):
    ...
    crop_id = sqlalchemy.Column(sqlalchemy.Integer, primary_key=True)
...
def get_crop_info(crop_name):
    ...
    with sqlalchemy.orm.Session(_engine) as session:
        query = session.query(Crop_Info).filter(
            Crop_Info.variety.ilike(crop_name+'%'))
        table = query.all()
        for row in table:
            crop_info = {'crop_id':row.crop_id, 'latin_name':row.latin_name, 'variety':row.variety,
